...
public class SaleChanceService extends BaseService<SaleChance,Integer> {
    ...
    private void checkParams(String customerName, String linkMan, String linkPhone) {
        AssertUtil.isTrue(StringUtils.isBlank(customerName),"请输入顾客姓名");
        AssertUtil.isTrue(StringUtils.isBlank(linkMan),"请输入联系人");
        AssertUtil.isTrue(StringUtils.isBlank(linkPhone),"请输入联系电话");
        AssertUtil.isTrue(!PhoneUtil.isMobile(linkPhone),"输入电话有误,请重新输入");
    }
    ...
    public void updateSaleChance(SaleChance saleChance){
         ...
         * 3.执行更新 判断结果
         */
        AssertUtil.isTrue(saleChance.getId()==null,"未选中待更新记录");
    ...
    public void deleteSaleChancesByIds(Integer[] ids){
        AssertUtil.isTrue(ids==null && ids.length<1,"请选择待删除的数据");
